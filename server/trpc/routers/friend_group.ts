...
export const friendgroupRouter = router({
      ...
      return {
        data: await ctx.prisma.friendGroup.update({
          where: {
            id: group.id,
            members: {
              some: { userId, OR: [{ role: 'FOUNDER' }, { role: 'USER' }] },
            },
          },
          data: {
            name: group.name,
  ...
  getAll: publicProcedure
    .query(async ({ ctx }) => {
      ...
      const groups = await ctx.prisma.friendGroup.findMany({
        where: { members: { some: { userId: (ctx.session.user as any).id } } },
        include: {
          members: {
            include: { user: true },
          },
  ...
  get: publicProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const groups = await ctx.prisma.friendGroup.findFirst({
        where: { id: input.groupId, members: { some: { userId: (ctx.session.user as any).id, } } },
        include: {
          members: {
            include: { user: true },
          },
  ...
  getFriendgroups: publicProcedure.query(async ({ ctx }) => {
    if (!ctx.session?.user) {
      return { error: 'Not authenticated' }
    }
    const groups = await ctx.prisma.friendGroup.findMany({
  ...
  getDefaultGroup: publicProcedure.query(async ({ ctx }) => {
    if (!ctx.session?.user) {
      return { error: 'Not authenticated' }
    }

    const group = await ctx.prisma.user.findUnique({
      where: { id: (ctx.session.user as any).id },
      select: {
        defaultFriendGroupId: true,
        defaultFriendGroup: true,
      },
    })

    if (!group) {
      return { error: 'No default group found' }
    }

    return { data: group.defaultFriendGroup }
  }),
})
